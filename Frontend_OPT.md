...
4. 业务自定义指标
   1. 页面中某些功能模块的加载时间（MP）
   2. 特定的用户行为路径完成时间（结合埋点）

### 设计的路径

不管之前有没有做过管理，虚线（实线）管理

#### 指标设计

1. 指标设计，
2. 采集，Performance API，Webvital，Span
3. 上报，图片，xhr，服务端还要Kafka削峰、Flink数据实时流处理、落库（日志型数据，列式存储 clickhouse、六边形战士Postgresql）
4. 数据清洗汇总
5. 数据统计分析，clickhouse物化视图
6. 可视化，echarts，antv，需要设计哪些可视化图表，然后再做可视化的渲染
